            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    ...
                    @elseif(!Auth::user()->isCliente())
                        <p class="mb-3">Solo clientes pueden comprar productos. Cambia a una cuenta de cliente o crea una desde el registro.</p>
                    @else
                        <form method="POST" action="{{ route('producto.comprar', $producto->slug) }}" id="form-comprar">
                            @csrf
                            <div class="mb-3">
                                <label class="form-label">Cantidad</label>
                                <input type="number" name="cantidad" id="cantidad-compra" class="form-control" min="1" value="{{ old('cantidad', 1) }}" {{ $producto->stock <= 0 ? 'disabled' : '' }}>
                                @error('cantidad')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="payment_method_id" id="payment-method-compra" class="form-control" required {{ $producto->stock <= 0 ? 'disabled' : '' }}>
                                    ...
                                    <option value="wayna_qr" data-qr="metodos/qr_wayna.jpg" data-codigo="" {{ old('payment_method_id') === 'wayna_qr' ? 'selected' : '' }}>Wayna QR (Escanear QR de la empresa)</option>
                                    <option value="transferencia_bancaria" data-qr="" data-codigo="" {{ old('payment_method_id') === 'transferencia_bancaria' ? 'selected' : '' }}>Transferencia bancaria</option>
                                    <option value="efectivo" data-qr="" data-codigo="" {{ old('payment_method_id') === 'efectivo' ? 'selected' : '' }}>Pago en efectivo</option>
                                    @foreach($metodosPago as $metodo)
                                        <option value="{{ $metodo->id_metodo }}" data-qr="{{ $metodo->qr_code ?? '' }}" data-codigo="{{ $metodo->codigo ?? '' }}" {{ old('payment_method_id') == $metodo->id_metodo ? 'selected' : '' }}>{{ $metodo->nombre }}</option>
                                    @endforeach
                                </select>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    @guest
                        ...
                        <a href="{{ route('login') }}" class="btn btn-wayna w-100">Ingresar</a>
                    @elseif(!Auth::user()->isCliente())
                        <p class="mb-3">Solo clientes pueden donar. Cambia a una cuenta de cliente o regístrate como cliente.</p>
                    @else
                        <form method="POST" action="{{ route('producto.donar', $producto->slug) }}" id="form-donar">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="monto" id="monto-donar" class="form-control" min="1" step="0.01" value="{{ old('monto', 10) }}" required>
                                @error('monto')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
